<template>
  <div class="container">
    <n-space vertical>
      <n-button v-if="isAdmin == 1" @Click="showModal = true">Create New Mission</n-button>
...
export default defineComponent({
  setup() {
    ...
    const missionNameReactive = reactive(missionName)
    const missionParamsReactive = reactive(missionParams)
    ...
    return {
      ...
      data: dataRef,
      columns: columnsRef,
      missionName: missionNameReactive,
      missionParams: missionParamsReactive,
      ...
      handleSorterChange(sorter) {
        if (!sorter || sorter.columnKey === 'missionName') {
          if (!loadingRef.value) {
            ...
            query(
              ...
              missionNameReactive.sortOrder = !sorter ? false : sorter.order
              dataRef.value = data.data
              paginationReactive.pageCount = data.pageCount
              paginationReactive.itemCount = data.total
              loadingRef.value = false
