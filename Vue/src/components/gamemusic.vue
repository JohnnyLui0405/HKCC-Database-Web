<template>
  <div class="container">
    <n-space vertical>
      <n-button v-if="isAdmin == 1" @Click="showModal = true">Create New Music</n-button>
      <n-modal v-model:show="showModal">
        <n-card style="width: 600px" title="Create New Music" :bordered="false" size="huge" role="dialog"
          aria-modal="true">
...
export default defineComponent({
  setup() {
    ...
    const musicNameReactive = reactive(musicName)
    const artistNameReactive = reactive(artistName)
    const genereReactive = reactive(genre)
    const bossCardIDReactive = reactive(bossCardID)
    const bossLevelReactive = reactive(bossLevel)
    ...
    return {
      ...
      data: dataRef,
      columns: columnsRef,
      musicName: musicNameReactive,
      artistName: artistNameReactive,
      genere: genereReactive,
      bossCardID: bossCardIDReactive,
      bossLevel: bossLevelReactive,
        ...
        if (!sorter || sorter.columnKey === 'name') {
          if (!loadingRef.value) {
            ...
            query(
              ...
              musicNameReactive.sortOrder = !sorter ? false : sorter.order
              dataRef.value = data.data
              paginationReactive.pageCount = data.pageCount
              paginationReactive.itemCount = data.total
              loadingRef.value = false
